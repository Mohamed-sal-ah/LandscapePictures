        ...
        <>
          <STYLED.HeaderAndExitView>
            <STYLED.HeaderAndTextView>
              <TouchableOpacity
                onPress={() =>
                  onNavigateUser(imageData.user_id, imageData.username)
                }
              >
                <STYLED.ProfilePictureView>
                  {users[`${imageData.user_id}`].profilePicture ? (
                    <STYLED.ProfilePicture
                      source={{
                        uri: users[`${imageData.user_id}`].profilePictureFile
                          ? users[`${imageData.user_id}`].profilePictureFile
                          : require(`../../storage/profileImage/${
                              users[`${imageData.user_id}`].profilePicture
                            }`),
                      }}
                    />
          ...
          <Image
            width={
              Math.floor(dimensions.width * 0.9) % 2 == 0
                ? Math.floor(dimensions.width * 0.9)
                : Math.floor(dimensions.width * 0.9) + 1
            }
            height={
              Math.floor(dimensions.height * 0.7) % 2 == 0
                ? Math.floor(dimensions.height * 0.7)
                : Math.floor(dimensions.height * 0.7) + 1
            }
            style={{ paddingBottom: 20 }}
            accessibilityLabel={
              imageData.hasOwnProperty("alt")
                ? imageData.alt
                : `Photo by ${imageData.username}`
            }
            source={{
              uri: imageData.hasOwnProperty("fileUrl")
                ? imageData.fileUrl
                : require(`../../storage/images/${imageData.fileName}`),
            }}
          />
          <STYLED.InfoAndDownLoadView>
            <STYLED.ImageInfoView>
              <STYLED.LoactionView>
                ...
                <STYLED.ImageInfoText style={{ color: theme.colors.black }}>
                  {imageData.location}
                </STYLED.ImageInfoText>
              </STYLED.LoactionView>
              <STYLED.ImageInfoText
                style={{ color: theme.colors.gray.dark, marginLeft: "36px" }}
              >
            ...
            <a
              ...
              href={
                imageData.hasOwnProperty("fileUrl")
                  ? imageData.fileUrl
                  : require(`../../storage/images/${imageData.fileName}`)
              }
              download={
                imageData.hasOwnProperty("fileUrl")
                  ? `${imageData.username}_${imageData.url_id}`
                  : `${imageData.username}_${imageData.fileName}`
              }
            >
