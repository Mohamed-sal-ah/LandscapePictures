  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {users[`${oneImage.user_id}`].profilePicture ? (
            <STYLED.ProfilePicture
              source={{
                uri: users[`${oneImage.user_id}`].profilePictureFile
                  ? users[`${oneImage.user_id}`].profilePictureFile
        ...
        <STYLED.UsernameText
          style={{ color: theme.colors.gray.dark, fontSize: 20 }}
        >
          {oneImage.username}
        </STYLED.UsernameText>
      </STYLED.MobileViewUserInfo>
      <Image
        ...
        height={500}
        accessibilityLabel={
          oneImage.hasOwnProperty("alt")
            ? oneImage.alt
            : `Photo by ${oneImage.username}`
        }
        style={{ minWidth: "100%", minHeight: 300 }}
        source={{
          uri: oneImage.hasOwnProperty("fileUrl")
            ? oneImage.fileUrl
            : require(`../../../storage/images/${oneImage.fileName}`),
        }}
      />
      ...
      >
        <>
          ...
          <a
            ...
            href={
              oneImage.hasOwnProperty("fileUrl")
                ? oneImage.fileUrl
                : require(`../../../storage/images/${oneImage.fileName}`)
            }
            download={
              oneImage.hasOwnProperty("fileUrl")
                ? `${oneImage.username}_${oneImage.fileName}`
                : `${oneImage.username}_${oneImage.fileName}`
            }
          >
      ...
      >
        ...
        <a
          ...
          download={
            oneImage.hasOwnProperty("fileUrl")
              ? `${oneImage.username}_${oneImage.url_id}`
              : `${oneImage.username}_${oneImage.fileName}`
          }
        >
          <ArrowDownSvg fillColor={theme.colors.gray.dark} />
